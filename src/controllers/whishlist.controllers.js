...
export const verifyWishlist = async (req, res) => {
    try {
        const { userId, productId } = req.params;
        if (!userId || !productId)
            return res.status(400).json({ msg: 'Uid and Pid both required' });

        const userFound = await User.findById(userId);
        if (!userFound) return res.status(404).json({ msg: 'User not found' });

        const isPresent = userFound.wishlist.some(item => item.productId.toString() === productId);

        return res.status(200).json({ isPresent });
    } catch (error) {
        return res.status(500).json({
            success: false,
            message: 'Error Verifying Wishlist',
            error,
        });
    }
};
